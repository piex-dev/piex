...
- **Throughput**: decode wall time (first token → last stream update) over `usage.output`, painted the moment the stream completes (`message_end` carries usage **before** tools run), so the bar is not stuck at TTFT-only during tool execution. Samples below 200ms of decode are suppressed: a buffering gateway that replays 1252 tokens in 97ms is noise, not a real rate.
