...
- Codex shows the ChatGPT subscription windows the backend exposes — account-level `primary_window` / `secondary_window` plus the **active model's own per-feature window** from `additional_rate_limits` (e.g. `Spark 5H:…%` when the model is `gpt-5.3-codex-spark`). Feature windows belonging to other models stay in `/usage` details only. `Credits:∞ / Credits:<balance>` appears when available; `Credits:available / Credits:none` is used as a fallback when no windows exist.
