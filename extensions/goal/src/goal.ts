...
function registerGoalRuntime(pi: ExtensionAPI, options: GoalOptions = {}) {
  ...
  const clearCompletionStatusTimer =
    runtime.clearCompletionStatusTimer.bind(runtime);
  const clearContinuationTracking =
    runtime.clearContinuationTracking.bind(runtime);
  ...
  const clearStaleGoalToolCallBlock =
    runtime.clearStaleGoalToolCallBlock.bind(runtime);
  ...
  const limitActiveGoalForBudget =
    runtime.limitActiveGoalForBudget.bind(runtime);
  ...
  const pauseGoalForUnavailableTools =
    runtime.pauseGoalForUnavailableTools.bind(runtime);
  const isPiOwnedCompactionRetry =
    runtime.isPiOwnedCompactionRetry.bind(runtime);
  const requestContinuation = runtime.requestContinuation.bind(runtime);
  const dispatchContinuationIfSettled =
    runtime.dispatchContinuationIfSettled.bind(runtime);
  ...
  const clearGoalRecoveryForGoal =
    runtime.clearGoalRecoveryForGoal.bind(runtime);
  ...
  const consumeStaleOwnedGoalPrompt =
    runtime.consumeStaleOwnedGoalPrompt.bind(runtime);
  ...
  const hasContinuationWorkForGoal =
    runtime.hasContinuationWorkForGoal.bind(runtime);
  ...
  const restoreGoalToolsHiddenByPolicy =
    runtime.restoreGoalToolsHiddenByPolicy.bind(runtime);
  const goalCompleteTool = defineTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (!runtime.canRecordGoalUsage() && !completingDuringBudgetWrapUp) {
        const rejection =
          "Goal completion rejected: current run does not own the active goal.";
        ctx.ui.notify(rejection, "warning");
        return {
          content: [{ type: "text", text: rejection }],
          details: {
      ...
      const staleGoalRejection = goalIdRejectionReason(
        completedGoal,
        requestedGoalId,
      );
  ...
  const goalBlockedTool = defineTool({
    ...
    parameters: Type.Object({
      goal_id: Type.String({
        description:
          "The exact goal_id shown in the current active /goal prompt.",
      }),
      reason: Type.String({
        minLength: 1,
        maxLength: MAX_BLOCKER_REASON_LENGTH,
        description:
          "The specific user or external action required to unblock the goal.",
      }),
      evidence: Type.String({
        minLength: 1,
        maxLength: MAX_BLOCKER_EVIDENCE_LENGTH,
        description:
          "Concrete evidence from the repeated attempts that proves the impasse.",
      }),
      repeated_turns: Type.Integer({
        minimum: 3,
        description:
          "Number of separate turns spent trying to resolve this same blocker.",
      }),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const repeatedTurns =
        typeof params.repeated_turns === "number"
          ? params.repeated_turns
          : Number.NaN;
  ...
  pi.registerCommand("goal", {
    description:
      "Run a goal to completion: /goal [--tokens 100k] <goal_to_complete>",
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    runtime.settings =
      settingsResult.kind === "loaded"
        ? settingsResult.settings
        : DEFAULT_GOAL_SETTINGS;
    if (settingsResult.kind === "invalid") {
      ctx.ui.notify(
        `goal settings ignored: ${settingsResult.reason}. Using default settings.`,
        "warning",
      );
    }
    if (
  ...
  pi.on("tool_call", (event, ctx) => {
    if (
      ...
      return {
        block: true,
        reason:
          "Goal token budget is exhausted; only goal_complete is allowed during wrap-up.",
      };
    }
    if (!runtime.staleGoalToolCallsBlocked) return;
    if (
      !runtime.activeGoal ||
      !blocksStaleGoalToolCalls(runtime.activeGoal.status)
    ) {
      clearStaleGoalToolCallBlock();
      return;
    }
  ...
  pi.on("agent_end", (event, ctx) => {
    ...
    runtime.agentRunGoalId = undefined;
    // canRecordGoalUsage() reads the (now-reset) field, so capture ownership
    // from the pre-reset value instead of re-querying a dead guard.
    const runOwnsGoal = agentRunGoalId !== null;
    if (agentRunGoalId === null) return;
    if (!runOwnsGoal && !runtime.hasActiveBudgetWrapUp()) return;
    ...
    if (!alreadyAwaitingContinuation)
    ...
    if (finalAssistant?.stopReason === "error") {
      if (isRetryableGoalInterruption(finalAssistant)) {
        ...
        runtime.goalRecovery = {
          goalId,
          kind: isGoalContextOverflow(finalAssistant)
            ? "compaction_retry"
            : "provider_retry",
        };
        cancelContinuationWork();
      ...
      stopGoalAfterAgentEnd(
        ...
        runtime.activeGoal,
        finalAssistant,
        isUsageLimitedGoalInterruption(finalAssistant)
          ? "usage_limited"
          : "blocked",
      );
      return;
