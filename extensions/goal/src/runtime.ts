...
import { type ActiveGoal, serializeGoalState } from "./persistence.js";
...
export class GoalRuntime {
  ...
  canRecordGoalUsage() {
    return this.agentRunGoalId !== null;
  }
  ...
  hasActiveGoalRecovery() {
    return Boolean(
      this.activeGoal && this.goalRecovery?.goalId === this.activeGoal.id,
    );
  }
  ...
  dispatchContinuationIfSettled(ctx: StatusContext) {
    ...
    // sendUserMessage returns Promise<void>; a sync try/catch cannot catch an
    // async rejection. Handle the promise explicitly so a delivery failure
    // clears the delivery ticket (otherwise hasContinuationWorkForGoal stays
    // true and requestContinuation can never re-arm, stalling the goal) and
    // re-arms the intent for a later agent_settled retry.
    this.pi
      .sendUserMessage(intent.prompt, { deliverAs: "followUp" })
      .catch((error) => {
  ...
  hasContinuationWorkForGoal(goalId: string) {
    return (
      this.continuationIntent?.goalId === goalId ||
      this.continuationDelivery?.goalId === goalId
    );
  }
  ...
  queueBudgetWrapUp(ctx: StatusContext, goal: ActiveGoal) {
    ...
    // sendMessage returns Promise<void>; handle the async rejection so a
    // delivery failure resets `delivered` (otherwise the idempotency guard
    // blocks all future re-queues) instead of leaving it stuck true.
    this.pi
      .sendMessage(
        {
      ...
      .catch((error) => {
        if (this.budgetWrapUp?.goalId === goal.id) {
          this.budgetWrapUp.delivered = false;
        }
        ctx.ui.notify(
          `Goal budget wrap-up failed: ${formatError(error)}`,
          "error",
        );
      });
    return true;
  ...
  limitActiveGoalForBudget(ctx: StatusContext, sendWrapUp: boolean) {
    ...
    ctx.ui.notify(
      `Goal token budget reached: ${formatBudget(this.activeGoal)}`,
      "warning",
    );
  ...
  cancelContinuationWork() {
    if (this.continuationDelivery) {
      this.rememberCancelledContinuationMarker(
        this.continuationDelivery.marker,
      );
    }
    this.continuationIntent = undefined;
    this.continuationDelivery = undefined;
  ...
  consumeStaleOwnedGoalPrompt(prompt: string) {
    ...
    if (this.activeGoal?.id === goalId && this.activeGoal.status === "active") {
      return false;
    }
  ...
  persistGoal(goal: ActiveGoal) {
    if (this.terminalDetails?.goalId !== goal.id) {
  ...
  restoreGoalToolsHiddenByPolicy() {
    ...
    try {
      ...
      if (missingOwnedTools.some((name) => !restored.has(name))) {
        throw new Error(
          "the active tool policy rejected a previously hidden goal tool",
        );
      }
      this.goalToolsHiddenByPolicy.clear();
    } catch (error) {
  ...
  prepareGoalToolsForActivation(ctx: StatusContext) {
    if (this.settings.toolVisibility === "after-first-goal") {
      if (!this.goalToolsAvailable() && ctx.isIdle?.() !== true) {
        throw new Error(
          "wait until Pi is idle before revealing the goal tools",
        );
      }
  ...
  private rememberCancelledContinuationMarker(marker: string) {
    ...
    if (
      this.cancelledContinuationMarkers.size <=
...
export function isContradictoryCompletionSummary(summary: string) {
  return CONTRADICTORY_COMPLETION_PATTERNS.some((pattern) =>
    pattern.test(summary),
  );
}

export function goalIdRejectionReason(
  goal: ActiveGoal,
  requestedGoalId: string,
) {
  if (!requestedGoalId) return "missing goal_id";
  if (requestedGoalId !== goal.id)
    return "goal_id does not match the active goal";
  return undefined;
}

export function isUsageLimitedGoalInterruption(
  ...
  return (
    ...
    USAGE_LIMIT_GOAL_ERROR_PATTERNS.some((pattern) =>
...
export function isRetryableGoalInterruption(assistant: AssistantMessageLike) {
  ...
  return (
    ...
    RETRYABLE_GOAL_ERROR_PATTERNS.some((pattern) =>
      pattern.test(assistant.errorMessage ?? ""),
    )
  );
}
