...
| `docs/<slug>/` · `blogs/<slug>/` | 旧 URL 重定向桩（按语言偏好 → `/{lang}/...`） |
...
| 规则 | 说明 |
|------|------|
| **只写中文 md** | 文档：`docs/<slug>.md`；博客：`docs/notes/<slug>.md` |
...
文档与博客均为**结构化 HTML**。Markdown 只作 Git 底稿与中文内容来源；线上只使用上表路径。

### 语言切换行为

- 主页：顶栏 EN / 中文 → `localStorage.piex-lang` + 字典替换；内容链接被改写为 `/{lang}/docs|blogs/...`
...
- 旧路径 `/docs/<slug>/`、`/blogs/<slug>/`：JS 按 `localStorage.piex-lang` → `navigator.language` → 默认 `en` 跳转；无 JS 时 meta refresh 到 `/en/...`
...
4. 旧路径重定向桩（新 slug）：`docs/docs/<slug>/index.html` → 语言感知跳转（默认 en）
5. `./scripts/check-docs-i18n.sh` 通过
6. 本地 server 验收 → commit / push
...
3. 同步各语言博客侧栏 + 主页 `#blog`
4. `./scripts/check-docs-i18n.sh` → 本地验收 → commit / push
