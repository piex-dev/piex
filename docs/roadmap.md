...
**已迁移 7 个 package（另含 xai-oauth 等）。** review 多 agent 版待定（依赖 subagent 机制）。subagent 已立项（见 [docs/packages/subagent.md](packages/subagent.md)），MVP 走子进程路线。
...
| 19  | **usage**      | 用量统计报告           | ➕ 功能增量 | —                                       | 展示各 provider 的 token 消耗、限额、速率重置，pi 有 token 追踪。✅ 已落地 usage（Kimi/Grok/Zhipu/MiniMax/DeepSeek 配额余额、Copilot 档位/限流、OpenAI Codex 订阅窗口/credits 状态栏，按模型显示），可继续扩展 provider |
...
### Tier 3 — 高复杂度 / 历史候选（原「深度耦合 omp」；部分已降级为 Extension 可实现）

| #   | 候选 Package           | 功能               | 一层      | 原因                                                                                                                                         |
| --- | ---------------------- | ------------------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 39  | **subagent**           | 子 agent 并行任务  | 🆕 新功能 | **已立项**（[docs/packages/subagent.md](packages/subagent.md)）：MVP 子进程委派原语；不再依赖 omp 内部引擎，从本表迁出优先级见下方「已立项」 |
...
| Package      | 一层      | 状态     | 说明                                                                                                      |
| ------------ | --------- | -------- | --------------------------------------------------------------------------------------------------------- |
| **subagent** | 🆕 新功能 | 方案已定 | 委派原语，subprocess MVP；见 [docs/packages/subagent.md](packages/subagent.md)。原 P2-5，不再等待 pi 内核 |
...
| 子 agent     | task orchestrator（完整）      | 刻意不做（No sub-agents）    | 已立项（子进程 MVP，见 packages/subagent.md） |
