...
| idle 空闲回收 | OMP `lsp/client.ts` | `PI_LSP_IDLE_TIMEOUT_MS`（默认 30min，0=关闭）+ 60s 扫描，回收僵尸 server 进程 |
...
| 生命周期         | 会话内复用（per root+server），in-flight spawn 去重，broken 标记；**idle 回收**（`PI_LSP_IDLE_TIMEOUT_MS` 默认 30min，60s 扫描） | 会话内复用（instance 级），broken + spawning 去重；无 idle 回收                                            | 会话内复用（per command:cwd），broken 标记；`idleTimeoutMs` 可选空闲回收（60s 扫描）                                   |
