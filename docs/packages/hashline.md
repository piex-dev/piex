...
type: extension
install: pi install npm:@piex-dev/hashline
...
**Hashline（锚定编辑）** 的回答是：把编辑坐标系从「第 N 行」升级为「我读到的那个版本 + 我见过的那些行」。`@piex-dev/hashline` 用它**覆盖** pi 内置 `edit`，让 agent 默认走更稳的补丁语言。
...
光有文件 tag 还不够：模型可能只 read 了文件前 80 行，却对第 200 行下手。oh-my-pi / piex 会从 read 输出的行号前缀解析 **seen-lines**，Patcher 拒绝编辑从未展示过的行（折叠摘要行 `12-40:` 只算边界，不算中间省略区）。
...
piex 在引擎外包了一层 `EditGuard` + `normalizeInput`（详见「实现方案」）。
...
> 仓库源码：[`extensions/hashline`](https://github.com/piex-dev/piex/tree/main/extensions/hashline)

### 配置

hashline 开箱即用，无必填配置。本地开发需先安装运行时依赖：
...
参数从「旧文本/新文本」类 schema 换成单一 `input` 字符串（DSL）。工具 description 直接读上游 `prompt.md`，保证语法说明与引擎一致。
