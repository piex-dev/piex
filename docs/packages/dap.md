...
npm: "@piex-dev/dap"
type: extension
install: pi install npm:@piex-dev/dap
...
多数 coding agent 的「修 bug」路径是这样的：读代码猜原因 → 改几行 → 用 bash 再跑一遍测试 → 失败就再猜。这在简单问题上够用，一碰到「只有运行时才暴露」的问题就很脆：空指针发生在第几层调用栈？某个局部变量当时到底是什么？断点打在条件分支上能不能停住？
...
`defaults.json` 里为每个 adapter 写了启动命令与参数、语言与文件后缀、项目根标记（如 `Cargo.toml`、`go.mod`、`package.json`）、launch / attach 默认参数。launch 时根据程序路径是文件还是目录、后缀、工作区根标记，自动挑一个可用 adapter。模型也可以显式指定 adapter 名称。

### 为什么是 stdio 而不是远程 TCP

omp 原版支持 stdio + TCP。piex 的 Node 移植**只做 stdio**：adapter 作为子进程，stdin/stdout 上跑 DAP 消息帧。取舍：本机调试主路径都是 stdio，少一套 socket 生命周期与鉴权；代价是不支持「连到远程机器上已开的 debug port」。
...
> 仓库源码：[`extensions/dap`](https://github.com/piex-dev/piex/tree/main/extensions/dap)
...
默认配置在 `extensions/dap/defaults.json`，每个 adapter 含启动命令、语言后缀、项目根标记、launch/attach 默认参数。无需手写配置，扩展按文件后缀 + 项目根标记自动选 adapter；模型也可显式指定 adapter 名称。
...
能力按 adapter 上报的 `capabilities` 做检查：不支持的操作直接报错，避免 silently no-op。
