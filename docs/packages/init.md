...
npm: "@piex-dev/init"
type: prompt
install: pi install npm:@piex-dev/init
source: prompts/init
---
...
Coding agent 进仓库的第一件事本该是读项目规则。现实里常是：**没有规则**（新项目根本没 `AGENTS.md`，模型只能猜 scripts 和目录边界）、**规则过时**（toolchain 已换，agent 仍按旧约定跑）、**迁移成本高**（已有 `CLAUDE.md`/Cursor rules 却没人整理成通用 `AGENTS.md`）、**入口不统一**（有人手写有人每次用自然语言，质量漂移）。

没有高信号规则，agent 更容易猜错测试命令、改错包边界、重复踩环境坑。`@piex-dev/init` 把这件事收成：`pi install` 一次，之后 `/init` 即可。

## 技术原理

### pi 只加载规则，不生成规则

pi 启动时从 `~/.pi/agent` 与 cwd 向上发现 `AGENTS.md`/`CLAUDE.md` 并注入 system prompt，但内置 slash 列表里没有 `/init`。这是极简设计：内核只做发现与注入；「怎么写出一份好规则」交给扩展或 prompt 包。

### Prompt template = 零代码的 slash 命令

pi package 可声明 `pi.prompts`。`prompts/init/init.md` 的文件名即命令名，frontmatter 提供 description 与 argument-hint，正文支持 `$ARGUMENTS`。用户输入 `/init …` 时，pi 展开模板为完整 user message，当前会话模型按指令调研并写文件。

### 价值在调查清单，不在模板长度

好的 init prompt 规定三件事：先读什么、提取什么、写什么/不写什么。可执行源（package.json scripts、CI、lockfile）优先于散文 README；已有规则就地改进，不盲目整文件覆盖。
...
> 仓库源码：[`prompts/init`](https://github.com/piex-dev/piex/tree/main/prompts/init)

### 用法

```bash
/init           # 扫描当前仓库并生成/改进 AGENTS.md
/init <说明>    # 带额外说明
```
...
纯 prompt 包，无 TypeScript、无 peer 依赖、无运行时代码。`package.json` 声明 `"pi": { "prompts": ["./init.md"] }`，`prompts/init/init.md` 即命令本体。
...
| 版本  | 日期       | 变更                                                                                                                                                                              |
| ----- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0.1.0 | 2026-07-19 | 初始版本：纯 prompt 包（`pi.prompts`），`/init` 扫描仓库生成/改进 `AGENTS.md`；调查清单（README/manifest/CI/lockfile 优先于散文）；就地改进已有规则不盲目覆盖；写完提示 `/reload` |
