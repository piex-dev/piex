...
从 oh-my-pi (omp) 提取 Plan Mode 和 Review 功能，迁移为 pie extension。

> **状态**: ✅ pie-plan 已实现 | ✅ pie-review 轻量版已实现 | ⏸ pie-review 多 agent 版待定
...
**文件**: `packages/pie-plan/extensions/plan.ts`
...
pi 本身不支持 sub-agent，但可以通过 extension 实现。pi 已有 [`subagent` 扩展示例](https://pi.dev)。对于 pie-review，有两种策略：

#### 策略 A: 轻量版 `pie-review`（推荐优先实现）
...
| 阶段 | 内容 | 状态 | 复杂度 |
|------|------|------|--------|
| **4a** | `pie-plan`（基于 pi 示例增强） | ✅ 已完成 | 中 |
| **4b** | `pie-review` 轻量版 | ✅ 已完成 | 中 |
| **4c** | `pie-review` 多 agent 版 | ⏸ 待定 | 高 |
...
|------|------|---------|---------|---------|
| pie-plan | pi 示例 + omp plan-mode | 低 | 无外部依赖 | 完全独立 |
| pie-review (轻量) | omp /review 命令 | 中 | 无外部依赖 | 完全独立 |
| pie-review (多agent) | omp 完整实现 | 高 | 需要 subagent 机制 | 依赖 pi subagent 扩展 |
