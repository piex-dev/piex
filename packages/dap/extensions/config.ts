...
function which(cmd: string): string | null {
  ...
  for (const dir of PATH.split(path.delimiter)) {
    for (const ext of exts) {
      ...
      try {
        if (fs.statSync(candidate).isFile()) { /* check exec on unix */ }
        return candidate;
...
function normalizeAdapterConfig(config: unknown): DapAdapterConfig | null {
  ...
  return {
    ...
    fileTypes: normalizeStringArray(config.fileTypes).map(e => e.toLowerCase()),
...
function getMatchingAdapters(program: string, cwd: string): DapResolvedAdapter[] {
  ...
  if (!extension) {
    ...
    return available.filter(
      adapter => nativeDebuggers.has(adapter.name) ||
...
export function selectLaunchAdapter(
  ...
  adapterName?: string, programKind: LaunchProgramKind = "file",
): DapResolvedAdapter | null {
  if (adapterName) return resolveAdapter(adapterName, cwd);
  const matches = getMatchingAdapters(program, cwd);
  const candidates = programKind === "directory"
    ? matches.filter(a => a.acceptsDirectoryProgram) : matches;
...
export function resolveLaunchOverrides(
  adapter: DapResolvedAdapter, program: string, programKind: LaunchProgramKind,
