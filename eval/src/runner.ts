...
program
  ...
  .option(
    "-a, --agents <list>",
    "agents to run (comma-separated: pi-bare,pi-piex,omp)",
    "pi-bare,pi-piex,omp",
  )
  .option(
    "-m, --model <model>",
  ...
  .action(async (opts) => {
    const agents = opts.agents.split(",").map((s: string) => s.trim()) as (
      "pi-bare" | "pi-piex" | "omp"
    )[];
    let tasks;
    ...
    console.log(
      report.agents
        .map(
          (a) =>
            `  ${a.agent}: ${a.resolveRate}% (${a.passedTasks}/${a.passedTasks + a.failedTasks})`,
        )
        .join("\n"),
    );
  });
