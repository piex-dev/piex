...
export class Sandbox {
  ...
  runCommand(cwd: string, command: string): { exitCode: number | null; stdout: string; stderr: string } {
    try {
      const stdout = execSync(command, { cwd, timeout: 60_000, encoding: 'utf-8', stdio: ['ignore', 'pipe', 'pipe'] })
...
async function runDocker(args: string[]): Promise<{ exitCode: number | null; stdout: string; stderr: string }> {
