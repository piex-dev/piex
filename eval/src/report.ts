...
function buildMarkdown(report: EvalReport): string {
  ...
  const headers = ['| 角色 | Agent | Resolve Rate | Passed/Failed | Avg Wall Time | Avg Tokens |',
    '|------|-------|-------------|---------------|---------------|------------|']
  ...
  for (const agent of report.agents) {
    ...
    const tokens = agent.avgTokens > 0 ? `${(agent.avgTokens / 1000).toFixed(0)}K` : 'N/A'
    lines.push(
  ...
  if (base && test) {
    ...
    const improvement = baseRate > 0 ? `+${Math.round((testRate - baseRate) / baseRate * 1000) / 10}%` : 'N/A'
    lines.push(`| resolve_rate | ${baseRate}% | ${testRate}% | **${improvement}** |`)

    const tokenImprovement = base.avgTokens > 0
      ? `${Math.round((test.avgTokens - base.avgTokens) / base.avgTokens * -1000) / 10}%`
      : 'N/A'
    lines.push(`| avg_tokens | ${base.avgTokens} | ${test.avgTokens} | ${tokenImprovement} |`)
  }
  ...
  for (const task of report.tasks) {
    const basePass = task.results[report.agents.find((a: AgentSummary) => a.role === 'baseline')?.agent ?? '']?.passed
    const testPass = task.results[report.agents.find((a: AgentSummary) => a.role === 'test')?.agent ?? '']?.passed
    const refPass = task.results[report.agents.find((a: AgentSummary) => a.role === 'reference')?.agent ?? '']?.passed

    const icon = (p: boolean | undefined) => p === true ? '✅' : p === false ? '❌' : '—'
    lines.push(
